...
# custom unwrapped version of the above benchmarking function
# returns milliseconds
def unwrapped_bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, mode, provider, device=DEVICE):
    assert mode in ["fwd", "bwd"]
    dtype = torch.float16
    if "triton" in provider:
        q = torch.randn((BATCH, H, N_CTX, HEAD_DIM), dtype=dtype, device=device, requires_grad=True)
...
def run_custom_fwd_bench(output_csv: Optional[pathlib.Path],is_fp8:bool):
    TOTAL_TOKS = 16384
    TOTAL_HEADS = 2048
    ...
    causals = [False, True]
    mode_str = 'fwd'
    provider_str = 'triton-fp8' if is_fp8 else 'triton'
    # return: (b, n, s, h)
    ...
    datatype_str = "FP8-FP32" if is_fp8 else "FP16-FP32"

    for is_causal in causals:
        for headdim in headdims:
            for seqlen in seqlens:
                batchsize, nheads, _, _ = get_full_shape(seqlen, headdim)
                time_ms = unwrapped_bench_flash_attention(batchsize, nheads, seqlen, headdim, is_causal, mode_str, provider_str)
                ...
                row = [datatype_str, "triton", batchsize, nheads, seqlen, headdim, int(is_causal), time_ms]
                rows.append(row)
...
if __name__ == "__main__":
    argparser = argparse.ArgumentParser(
        prog="triton attention bench"
    )
    argparser.add_argument("output_csv", type=str, default=None, nargs='?')
    argparser.add_argument("--fp8", action="store_const", default = False, const = True)
    args = argparser.parse_args()
    output_csv = pathlib.Path(args.output_csv) if args.output_csv else None
    is_fp8 = args.fp8
    if is_fp8:
        print("=== BENCHMARK FP8-FP32 ===")
    else:
        print("=== BENCHMARK FP16-FP32 ===")
    run_custom_fwd_bench(output_csv, is_fp8)
