...
std::tuple<std::vector<std::string>, std::vector<std::vector<std::string>>> loop_bench_hdim128_c1()
{
  ...
  constexpr auto head_dims = cute::make_tuple(Int<128>{});
  ...
  constexpr auto causal_list = cute::make_tuple(cute::true_type{});
  ...
  for_each(make_int_sequence<causal_list_len>{}, [&](auto causal_idx){
    ...
    for_each(make_int_sequence<head_dims_len>{}, [&](auto num_head_head_dim_idx){
      ...
      for_each(make_int_sequence<seqlens_len>{}, [&](auto batchsize_seqlen_idx){
        ...
        float avg_time_ms = bench_fwd_fp16<head_dim_v, is_causal_v, 128, 64>(
          batch_size_v,
          num_head_v,
          seqlen_v,
          iter,
