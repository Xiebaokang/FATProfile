...
def _warning_blocks(output: str) -> tuple[set[str], dict[str, list[str]]]:
    matches = list(BUILD_BLOCK_RE.finditer(output))
    compiled_targets: set[str] = set()
    warnings: dict[str, list[str]] = {}
    for index, match in enumerate(matches):
        target = match.group(1)
        compiled_targets.add(target)
        end = matches[index + 1].start() if index + 1 < len(matches) else len(output)
        block = output[match.start():end]
        target_warnings = [item.group(0) for item in PTXAS_PERF_LOSS_RE.finditer(block)]
        if target_warnings:
            warnings.setdefault(target, []).extend(target_warnings)
    return compiled_targets, warnings
...
def build_source(
    ...
    *,
    ...
    build = [
        "cmake", "--build", str(level_dir), "--target", *targets,
        "--parallel", str(min(jobs, len(targets))),
    ]
    ...
    # A failed batch is retried target-by-target. Successful targets are then
    # no-ops, while the retry gives an exact failure status for the bad ones.
    target_ok = {target: process.returncode == 0 for target in targets}
    outputs = [output]
    if process.returncode != 0:
        for target in targets:
            ...
            retry_process = subprocess.run(
                retry, text=True, capture_output=True, check=False
            )
            retry_output = "\n".join(
                part for part in (
                    retry_process.stdout.strip(), retry_process.stderr.strip()
                ) if part
            )
            outputs.append(retry_output)
            target_ok[target] = retry_process.returncode == 0
            if retry_process.returncode != 0:
                print(f"compile failed: {target}\n{retry_output}")

    combined_output = "\n".join(part for part in outputs if part)
    compiled_targets, warning_map = _warning_blocks(combined_output)
    all_warnings = [item.group(0) for item in PTXAS_PERF_LOSS_RE.finditer(combined_output)]
    assigned_warnings = sum(len(items) for items in warning_map.values())
    if len(all_warnings) > assigned_warnings:
        # The output format is generator-dependent. If a warning cannot be
        # attributed to a build block, exclude the whole batch rather than
        # accidentally benchmarking a known performance-loss kernel.
        for target in targets:
            warning_map.setdefault(target, []).extend(all_warnings)
    ...
    for target in targets:
        ...
        warnings = warning_map.get(target, [])
        old = previous.get(target)
        if (
            not warnings
            and target not in compiled_targets
            and old is not None
        ...
        if not target_ok[target] or not executable.is_file():
            status = "compile_failed"
        elif warnings:
            status = "performance_warning"
            print(f"excluded {target}: ptxas potential performance loss warning")
        else:
            status = "success"
