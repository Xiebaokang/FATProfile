...
def run_fp16_fa_sdpa(q_tensor, k_tensor, v_tensor):
    q_tensor_fp16 = q_tensor.to(torch.float16).to("cuda")
    k_tensor_fp16 = k_tensor.to(torch.float16).to("cuda")
    v_tensor_fp16 = v_tensor.to(torch.float16).to("cuda")

    (batchsize, nheads, seqlen, headdim) = q_tensor_fp16.shape
    ...
    return attention_output.to("cpu")

def main():
    argparser = argparse.ArgumentParser(description="Run SDPA with FP16 FlashAttention backend")
    ...
    output_tensor = run_fp16_fa_sdpa(q_tensor, k_tensor, v_tensor)
