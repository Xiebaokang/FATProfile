...
def run_fp16_math_sdpa(q_tensor, k_tensor, v_tensor):
    q_tensor_fp16 = q_tensor.to(torch.float16)
    k_tensor_fp16 = k_tensor.to(torch.float16)
    v_tensor_fp16 = v_tensor.to(torch.float16)
...
def main():
    argparser = argparse.ArgumentParser(description="Run SDPA with FP16 math backend")
    ...
    output_tensor = run_fp16_math_sdpa(q_tensor, k_tensor, v_tensor)
