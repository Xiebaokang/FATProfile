...
#include "custom_data_dump.cuh"
#include "custom_smem_init.cuh"
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_M, bool Is_even_K, bool Is_softcap, bool Return_softmax, typename Params>
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    // TODO(rbxu): The first SMEM init method. The below "useVecFill" is a tunable for both performance and correctness (ideally it should be true).
    constexpr bool useVecFill = true;
    bool filled = SmemFiller::init_smem_q<useVecFill, Is_even_MN, Is_even_M, kBlockM, kNWarps, Element>(
        sQ, binfo.actual_seqlen_q, tidx
    );
    // // SmemFill debug print
    // __syncthreads();
    // if (tidx == 0 && filled && bidb == 0 && bidh == 0) {
    //     print_tensor(sQ);
    // }
    // __syncthreads();
    ...
    // TODO(rbxu): FA2 people actually have implemented some logics to fill the out-of-bound values when writing to Q SMEM (sQ)
    // And the below "Clear_OOB_MN" is for controlling this behavior, which defaults to false.
    // Here, I have exposed this switch and set it to false.
    // Ideally, this "Clear_OOB_MN" should be mutually exclusive with our custom SmemFiller::init_smem_q,
    // Which means: it should be sufficient to use exactly one of them.
    // You could:
    // Step1: Keep everyhing intact, try my smem init method
    // Step2: Comment out the SmemFiller::init_smem_q, and set Clear_OOB_MN to true.
    // Step3: If both methods could work, then you could compare the performance between them.
    constexpr bool Clear_OOB_MN = false;
    FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K, Clear_OOB_MN>(gmem_tiled_copy_QKV, tQgQ, tQsQ, tQcQ, tQpQ,
    ...
    using AccSTraits = FlashFwdAccSTensorTraits<Kernel_traits>;
    using RowMaxTraits = FlashFwdRowMaxTensorTraits<Kernel_traits>;
    using AccOTraits = FlashFwdAccOTensorTraits<Kernel_traits>;
    ...
    #if USE_MMA_SOFTMAX
    constexpr float warpmax_offset = -75.f;
    FLASH_NAMESPACE::MmaSoftmax<AccSTraits, AccOTraits, Is_even_MN, Is_even_M> softmax(
        binfo.actual_seqlen_q, params.scale_softmax_log2, tidx, warpmax_offset
    );
    #else
    ...
    #if ENABLE_CUSTOM_DUMP_AND_PRINTF
    // report the number of masking steps
    if (cute::thread0()){
        printf("Custom Report: n_masking_step: %d, n_block_min: %d, n_block: %d\n", n_masking_steps, n_block_min, n_block);
    }
    __syncthreads();
    #endif
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        ////// CUSTOM: manully CSE apply_mask_row_idx parameter
        ////// CUSTOM: this will also be used for our custom MmaSoftmax
        int apply_mask_row_idx = m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4;
        mask.template apply_mask<Is_causal, Is_even_MN>(
            acc_s, n_block * kBlockN, apply_mask_row_idx, kNWarps * 16
        );

        #if USE_ACC_S_LEVEL_INF_MASKING
        apply_mma_mask<Is_even_MN, Is_even_M, AccSTraits>(
            acc_s, binfo.actual_seqlen_q, apply_mask_row_idx
        );
        #endif
        ...
        #if ENABLE_CUSTOM_DUMP_AND_PRINTF
        dump_acc_s<AccSTraits>(
            gridDim.x, // m_block_num
            n_block_max,
            gridDim.y, // num_batch
            gridDim.z, // num_heads
        ...
        #if USE_MMA_SOFTMAX
        masking_step == 0
            ? softmax.template softmax_rescale_o</*Is_first=*/true,  /*Check_inf=*/Is_causal || Is_local>(acc_s, acc_o, params.scale_softmax_log2, apply_mask_row_idx)
            : softmax.template softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_causal || Is_local>(acc_s, acc_o, params.scale_softmax_log2, apply_mask_row_idx);
        #else
        ...
        #if ENABLE_CUSTOM_DUMP_AND_PRINTF
        dump_acc_s_after_softmax<AccSTraits>(
            gridDim.x, // m_block_num
            n_block_max,
            gridDim.y, // num_batch
            gridDim.z, // num_heads
        ...
        #if ENABLE_CUSTOM_DUMP_AND_PRINTF
        auto& dump_target_row_max = softmax.row_max;
        dump_row_max<RowMaxTraits>(
            gridDim.x, // m_block_num
            n_block_max,
            gridDim.y, // num_batch
            gridDim.z, // num_heads
        ...
        // #if ENABLE_CUSTOM_DUMP_AND_PRINTF
        // auto& dump_target_row_sum = softmax.row_sum;
        // dump_row_sum<RowMaxTraits>(
        //     gridDim.x, // m_block_num
        //     n_block_max,
        //     gridDim.y, // num_batch
        //     gridDim.z, // num_heads
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        #if USE_MMA_SOFTMAX
        softmax.template softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_local>(acc_s, acc_o, params.scale_softmax_log2, apply_mask_row_idx);
        #else
...
inline __device__ void compute_attn(const Params &params) {
    ...
    FLASH_NAMESPACE::compute_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_M, Is_even_K, Is_softcap, Return_softmax>(params, bidb, bidh, m_block);
}
